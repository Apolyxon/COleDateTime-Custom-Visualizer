    ...
    )
{
    ...
    // Read the COleDateTime value from the target process
    ...
    CString strValue;
    hr = COleDateTimeToText(value, /*ref*/strValue);
    if (FAILED(hr))
    {
    ...
    )
{
    ...
    // the result of the expression is a COleDateTime, and we don't want our visualizer to be invoked again. This
...
{
    // COleDateTime doesn't have an underlying string (no DkmEvaluationResultFlags::RawString), so this method
    // doesn't need to be implemented
    return E_NOTIMPL;
}

HRESULT CCppCustomVisualizerService::COleDateTimeToText(const COleDateTime& dateTime, CString& text)
{
